...
const withinBudget = async (work, budgetMs) => { let timer; try { return await Promise.race([work, new Promise((_, reject) => { timer=setTimeout(() => reject(Object.assign(new Error("The response took too long."), { status: 504 })), budgetMs); })]); } finally { clearTimeout(timer); } };
export async function orchestrate({ ai, message, memories, conversation, authorization, mode = "normal" }) {
  ...
  try { const response = await withinBudget(converse(ai, message, buildContext({ memories, conversation })), decision.complexity.budgetMs);
  ...
  } catch { return { type: "conversation_unavailable", answer: "I heard you, but I couldn't finish that reply right now. Please try again in a moment.", modelUsed: false, degraded: true, context, intentProtection, processing: decision.processing, timing: timing() }; }
}
