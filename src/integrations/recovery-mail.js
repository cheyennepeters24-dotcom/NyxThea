...
export async function sendRecoveryMail(env,{to,kind,code}){
  ...
  const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),6000);let response;try{response=await fetch("https://api.resend.com/emails",{method:"POST",signal:controller.signal,headers:{"content-type":"application/json","authorization":`Bearer ${env.RESEND_API_KEY}`},body:JSON.stringify({from:env.NYXTHEA_RECOVERY_FROM,to:[to],subject,html,tags:[{name:"category",value:kind==="verify"?"recovery_email_verify":"account_recovery"}]})})}finally{clearTimeout(timer)};
