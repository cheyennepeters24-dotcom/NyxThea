...
function normalizeMembership(profileId){
  ...
  const kept=new Set(),seenHouseholds=new Set();
  for(const [key,membership] of ranked)if(!seenHouseholds.has(membership.householdId)){kept.add(key);seenHouseholds.add(membership.householdId);}
  for(const [key] of active)if(!kept.has(key))memberships().delete(key);
  if(chosen){const memberIds=new Set(list("household_memberships",m=>seenHouseholds.has(m.householdId)&&m.active!==false).map(m=>m.profileId));for(const [key,relationship] of relationships())if((relationship.fromProfileId===profileId||relationship.toProfileId===profileId)&&(!memberIds.has(relationship.fromProfileId)||!memberIds.has(relationship.toProfileId)))relationships().delete(key);return chosen[1];}
  return null;
}
