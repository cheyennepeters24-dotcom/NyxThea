...
import { withDurableState } from "./src/state/durable-store.js";
...
async function api(request, env, url) {
  ...
  const profile = own(request, `${request.method} ${url.pathname}`); const memory = memoryService("local-user", profile.id, { durable: Boolean(env.NYXTHEA_DB) });
