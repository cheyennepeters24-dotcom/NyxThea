...
import { assessWakeContext, transitionVoice, voiceState, voicePlan } from "./src/architecture/voice.js";
...
async function api(request, env, url) {
  ...
  if (request.method === "POST" && url.pathname === "/api/auth/register") { authLimit(request, "register", 3, 86400000); const result = await registerAccount(await readJson(request)); return json({ profile: result.profile, recoveryCode: result.recoveryCode }, 201, { "set-cookie": sessionCookie(result.token) }); }
